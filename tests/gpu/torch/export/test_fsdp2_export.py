...
def _tied_map_survives_fsdp2_test(rank, size):
    ...
    ``fully_shard`` splits the shared ``nn.Parameter`` into distinct per-module shards, so
    ``id``-based detection goes empty afterwards. The HF map is a plain name dict on the model,
    unaffected by sharding, so TiedWeightMap still resolves the tie -- no pre-shard capture needed.
    """
    with patch_fsdp_mp_dtypes():
        ...
        # Pre-shard: both the HF map and id-based detection resolve the tie.
        ...
        # Post-shard: id-based detection goes EMPTY (the shared Parameter was split)...
        assert _build_tied_alias_map(model) == {}
        # ...but the HF name-based map is untouched -> TiedWeightMap still resolves the tie.
